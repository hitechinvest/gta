...
import { buildNet, netPoint, netAdvance, netSpawn, NET_WALK } from '/shared/roadnet.js';
...
export class Peds {
  ...
  /** Стартовое место на тротуаре: случайное ребро сети в кольце вокруг игрока. */
  spawnOnNet(cx, cz) {
    const state = netSpawn(this.net, cx, cz, 30, SPAWN_RADIUS);
    if (!state) return;
